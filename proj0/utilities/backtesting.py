...
def basic_single_asset_backtest(trades, days):
    ...
    df_trades["trade_result_pct"] = (
        df_trades["trade_result"] / df_trades["open_trade_size"]
    )
    ...
    buy_and_hold_pct = (
        df_days.iloc[-1]["price"] - df_days.iloc[0]["price"]
    ) / df_days.iloc[0]["price"]
    ...
    sharpe_ratio = (365**0.5) * (
        df_days["daily_return"].mean() / df_days["daily_return"].std()
    )

    best_trade = df_trades["trade_result_pct"].max()
    best_trade_date1 = str(
        df_trades.loc[df_trades["trade_result_pct"] == best_trade].iloc[0]["open_date"]
    )
    best_trade_date2 = str(
        df_trades.loc[df_trades["trade_result_pct"] == best_trade].iloc[0]["close_date"]
    )
    worst_trade = df_trades["trade_result_pct"].min()
    worst_trade_date1 = str(
        df_trades.loc[df_trades["trade_result_pct"] == worst_trade].iloc[0]["open_date"]
    )
    worst_trade_date2 = str(
        df_trades.loc[df_trades["trade_result_pct"] == worst_trade].iloc[0][
            "close_date"
        ]
    )
    ...
    print(
        "Worst Drawdown T|D: -{}% | -{}%".format(
            round(max_trades_drawdown * 100, 2), round(max_days_drawdown * 100, 2)
        )
    )
    ...
    print(
        "\nBest trades: +{} % the {} -> {}".format(
            round(best_trade * 100, 2), best_trade_date1, best_trade_date2
        )
    )
    print(
        "Worst trades: {} % the {} -> {}".format(
            round(worst_trade * 100, 2), worst_trade_date1, worst_trade_date2
        )
    )
...
def basic_multi_asset_backtest(trades, days):
    ...
    df_trades["trade_result_pct"] = df_trades["trade_result"] / (
        df_trades["open_trade_size"] + df_trades["open_fee"]
    )
    ...
    print("-" * 95)
    print(
        "{:<6s}{:>10s}{:>15s}{:>15s}{:>15s}{:>15s}{:>15s}".format(
            "Trades",
            "Pair",
            "Sum-result",
            "Mean-trade",
    ...
    for pair in df_trades["pair"].unique():
        ...
        pair_win_rate = (
            str(round((pair_good_trades / pair_total_trades) * 100, 2)) + " %"
        )
        ...
        print(
            "{:<6d}{:>10s}{:>15s}{:>15s}{:>15s}{:>15s}{:>15s}".format(
                pair_total_trades,
                pair,
                pair_sum_result,
                pair_avg_result,
...
def plot_sharpe_evolution(df_days):
    ...
    df_days_copy["daily_return"] = df_days_copy["evolution"] / df_days_copy[
        "wallet"
    ].shift(1)
...
def get_metrics(df_trades, df_days):
    ...
    sharpe_ratio = (365**0.5) * (
        df_days_copy["daily_return"].mean() / df_days_copy["daily_return"].std()
    )
    ...
    df_trades_copy["trade_result_pct"] = (
        df_trades_copy["trade_result"] / df_trades_copy["open_trade_size"]
    )
...
def plot_bar_by_month(df_days):
    ...
    while current_year != last_year or current_month - 1 != last_month:
        ...
        monthly_perf = (
            df_days.loc[date_string]["wallet"].iloc[-1]
            - df_days.loc[date_string]["wallet"].iloc[0]
        ) / df_days.loc[date_string]["wallet"].iloc[0]
        monthly_row = {
        ...
        if monthly_row["result"] >= 0:
            custom_palette[
                str(datetime.date(1900, current_month, 1).strftime("%B"))
            ] = "g"
        else:
            custom_palette[
                str(datetime.date(1900, current_month, 1).strftime("%B"))
        ...
        if (current_month == 12) or (
            current_month == last_month and current_year == last_year
        ):
            current_df = pd.DataFrame(current_year_array)
            # print(current_df)
            g = sns.barplot(
                data=current_df, x="date", y="result", palette=custom_palette
            )
            ...
            year_result = (
                df_days.loc[str(current_year)]["wallet"].iloc[-1]
